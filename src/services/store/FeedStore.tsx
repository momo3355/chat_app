...
import { FeedState, FeedActions } from '../../types/FeedTypes';
import { getFeedList, createFeed, toggleFeedLike } from '../api/FeedApi';
...
const useFeedStore = create<FeedStore>((set, get) => ({
  ...
  loadMore: async (userId: string) => {
    ...
    const res = await getFeedList(userId, nextPage, PAGE_SIZE);
    if (res.success) {
      set({
